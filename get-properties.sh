...
function runTool() {
  ...
  # ADDED: source setenv.sh and export KYLIN_EXTRA_START_OPTS
  source ${KYLIN_HOME}/sbin/setenv.sh
  export KYLIN_EXTRA_START_OPTS=`echo ${KYLIN_EXTRA_START_OPTS}|sed  "s/-XX:+PrintFlagsFinal//g"`
  local result=$(java ${KYLIN_EXTRA_START_OPTS} ${KYLIN_KERBEROS_OPTS} -Dlog4j.configurationFile=${kylin_tools_log4j} -Dkylin.hadoop.conf.dir=${kylin_hadoop_conf_dir} -Dhdp.version=current -cp "${kylin_hadoop_conf_dir}:${KYLIN_HOME}/lib/ext/*:${KYLIN_HOME}/server/jars/*:${SPARK_HOME}/jars/*" "$@" 2>>${KYLIN_HOME}/logs/shell.stderr)
  echo "$result"
}
